...
public class EventConsumerConfig {
    ...
    // event resived [PaymentEvent]
    @Bean  // been expose to spring function application.yml 
    public Consumer<PaymentEvent> paymentEventConsumer(){
        ...
        return (payment)-> 
        		handler.updateOrder(payment.getPaymentRequestDto().getOrderId(),po->{
        					po.setPaymentStatus(payment.getPaymentStatus());  //PAYMENT_COMPLETED | PAYMENT_FAILED
        		});
    }
}
