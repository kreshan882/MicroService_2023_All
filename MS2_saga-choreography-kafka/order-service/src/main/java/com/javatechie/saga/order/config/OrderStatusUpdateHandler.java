...
public class OrderStatusUpdateHandler {
    ...
    public void updateOrder(int id, Consumer<PurchaseOrder> consumer) {
        repository.findById(id).ifPresent(consumer.andThen(this::updateOrder)); // if payment.getPaymentRequestDto().getOrderId() avaliable
    }

    private void updateOrder(PurchaseOrder purchaseOrder) {
    	
        boolean isPaymentComplete = PaymentStatus.PAYMENT_COMPLETED.equals(purchaseOrder.getPaymentStatus()); // check payment ctatus
        OrderStatus orderStatus = isPaymentComplete ? OrderStatus.ORDER_COMPLETED : OrderStatus.ORDER_CANCELLED;
        
        purchaseOrder.setOrderStatus(orderStatus); // update order status
        
        if (!isPaymentComplete) { // payment not complete in payment-service
