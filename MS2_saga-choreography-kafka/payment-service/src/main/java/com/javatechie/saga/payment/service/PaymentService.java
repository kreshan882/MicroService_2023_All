...
public class PaymentService {
    ...
    //user id and amount hardcoded to db table
    @PostConstruct
    ...
    public PaymentEvent newOrderEvent(OrderEvent orderEvent) {
    	...
    	OrderRequestDto orderRequestDto = orderEvent.getOrderRequestDto(); // condume the object from kafka supplier

        PaymentRequestDto paymentRequestDto = new PaymentRequestDto(orderRequestDto.getOrderId(),   //return object created
                													orderRequestDto.getUserId(), 
                													orderRequestDto.getAmount());

        return userBalanceRepository.findById(orderRequestDto.getUserId())  // get user|avaliable UB User Balance object
                .filter(ub -> ub.getPrice() > orderRequestDto.getAmount())  // check condition avaliableAmy> Trxn Amt
                .map(ub -> { // amount avaliable
                    
                	ub.setPrice(ub.getPrice() - orderRequestDto.getAmount()); //user balance table updated
                    ...
                    return new PaymentEvent(paymentRequestDto, PaymentStatus.PAYMENT_COMPLETED); //return PaymentEvent
                
                })
                .orElse( //amount not enofe
